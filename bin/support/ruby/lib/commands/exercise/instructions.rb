...
module Exercise
  class Output < String
    ...
    class CICOutput
      ...
      def initialize(string)
        @container_id = string[/Starting container: (.*)/, 1]
        @cic_start_command = string[/(cic start .*)/, 1]
        @cic_connect_command = string[/(cic connect .*)/, 1]
        @cic_stop_command = string[/(cic stop .*)/, 1]
      end
    end
